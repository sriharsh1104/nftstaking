...
contract NFTStaking is ReentrancyGuardUpgradeable, Ownable {
    ...
    mapping(uint256 => uint256) public baseRates;
    ...
    function claimRewards() external {
        ...
        for (uint256 i = 0; i < _stakedTokensLength; i++) {
            stakers[msg.sender].stakedTokens[i].timeOfLastClaim = block.timestamp;
        }
        ...
        returns (uint256 _rewards)
    {
        ...
        uint256 _stakedTokensLength = stakers[_staker].stakedTokens.length; 
         for (uint256 i = 0; i < _stakedTokensLength; i++) {
            ...
            uint256 timeOfLastStake = stakers[_staker].stakedTokens[i].timeOfLastStake;

            // Get time of Last Claim
            uint256 timeOfLastClaim = stakers[_staker].stakedTokens[i].timeOfLastClaim;

            // Get the user's current Rate 
            // TODO: Call to get this tokenId's base rate from external source
            uint256 rate = (baseRates[stakers[_staker].stakedTokens[i].tokenId] * 1e18);

            if (withdrawPeriod < block.timestamp - timeOfLastStake) {
                if ((withdrawPeriod - (timeOfLastClaim - timeOfLastStake)) > 0) {
                ...
                _rewards += ((block.timestamp - timeOfLastClaim) * rate) / 86400; 
            }
        }

        // If the user is a soul owner, give them a 10% bonus
        if(soulOwner[msg.sender]) {
            _rewards = (_rewards * soulPercentage) / 10;
        }
    ...
    function stakeInternalLogic(uint256 _tokenId) internal {
        ...
        // Create StakedToken
        StakedToken memory stakedToken = StakedToken(_tokenId, block.timestamp, block.timestamp);
    ...
    function setSoulOwners(SoulOwner[] memory _newSoulOwners) public onlyOwner {
        // Get each object passed in to setBaseRate
         for (uint256 i = 0; i < _newSoulOwners.length; i++) {
            // Add Mapping for each object
    ...
    function setTreasuryPercentage(uint256 _newtreasuryPercentage) public onlyOwner {
        treasuryPercentage = _newtreasuryPercentage; 
    }

    // Function to set Soul Percentage
    function setSoulPercentage(uint256 _newtsoulPercentage) public onlyOwner {
